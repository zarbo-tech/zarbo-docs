...
var useThemeAuthor = () => {
  ...
  return computed(() => Lt(themeLocale.value.author, false));
};
...
var useAutoLink = (item, preferFull = false) => {
  const router = useRouter();
  const { fullPath, meta, name } = Re(router, encodeURI(item));
  return {
...
var usePageAuthor = () => {
  ...
  return computed(() => {
    ...
    if (author === false)
      return [];
    return Lt(themeLocale.value.author, false);
  });
};
var usePageCategory = () => {
  const frontmatter = (0, client_exports.usePageFrontmatter)();
  return computed(() => Wt(frontmatter.value.category).map((name) => {
    var _a, _b;
    return {
      name,
...
var usePageTag = () => {
  const frontmatter = (0, client_exports.usePageFrontmatter)();
  return computed(() => Rt(frontmatter.value.tag).map((name) => {
    var _a, _b;
    return {
      name,
...
var usePageDate = () => {
  ...
  return computed(() => {
    ...
    if (createdTime)
      return Te(new Date(createdTime));
    return null;
  });
};
var usePageInfo = () => {
...
//# sourceMappingURL=chunk-4BT6XTJG.js.map
